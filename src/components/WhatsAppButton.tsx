...
export function WhatsAppButton({ className }: WhatsAppButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className={cn(
        "group relative flex flex-col items-center justify-center w-full max-w-lg py-8 px-6 bg-whatsapp rounded-2xl glow-whatsapp transition-all duration-300 hover:scale-105 active:scale-95 overflow-hidden",
        className
      )}
    >
      ...
      <p className="text-white/90 text-sm md:text-base font-medium text-center">
        Clique para falar com um consultor e garantir sua vaga
      </p>
    </button>
  );
}
